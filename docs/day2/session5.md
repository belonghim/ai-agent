...
**시나리오:** 사용자가 "엔비디아(Nvidia)의 주가 흐름과 주요 뉴스를 찾아서 투자 의견을 써줘"라고 하면, 에이전트가 **웹 검색 → 뉴스 읽기 → 분석 → 리포트 작성**을 수행합니다.
...
* **Prompt Instructions (System):** (Expression 모드)
  ...
  - 오늘({{ $now.format('yyyy년 MM월 dd일') }})을 기준으로 정보 제공.
  - 동일 키워드 사용 금지.
  ...
  * 1단계. 탐색 (google_search)
    - 다음 키워드 각각 호출: ".. today market cap stock price" , ".. today stock news analysis"
