...
## Session 3: n8n 워크플로우 이것은 알고 시작하자 (기본 다지기 & Local LLM 연동)
...
워크플로우의 데이터는 **'Item'**이라는 택배 상자에 담겨 컨베이어 벨트(선)를 타고 흐릅니다.
...
2. **워크플로우**은 들어온 데이터 개수(Item)만큼 알아서 반복해서 일처리를 한다. (Loop 불필요)
