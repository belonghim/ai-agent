...
* **Prompt Instructions (System):** (Expression 모드)
  ...
  4. 주가 히스토리나 차트 데이터가 필요할 때는, 특정 사이트에 접속하기보다 구글 검색창에 "... stock price history text"와 같이 검색하여, 검색 결과 목록에서 데이터를 추출하는 것을 선호하세요.

  [출력 규칙]
  1. 언어: 검색 결과가 영어라도, 최종 리포트와 이메일 본문은 반드시 '한국어'로 번역하여 작성하세요.
