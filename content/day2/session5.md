...
* **Prompt Instructions (System):** (Expression 모드)
  ...
  * 1단계. 탐색 (google_search)
    - 키워드: "{종목명} recent stock price market cap text", "{종목명} stock news analysis forecast"
    - 주가 데이터는 탐색 요약 내용에서 추출하세요.
    - 2단계 읽기 도구를 호출한 적 없다면 리포트를 작성하지 마세요.
  
  * 2단계. 읽기 (web_scraper)
    - Javascript 나 Asp 기반 동적 사이트(finance.yahoo, marketwatch 등)는 피하고, 텍스트 위주의 뉴스 기사를 선택하세요.
    - 탐색 결과에서 가장 중요해 보이는 기사 1개만 찾아 내용을 읽기를 시도하세요.
