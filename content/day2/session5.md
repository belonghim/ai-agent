...
* **Prompt Instructions (System):** (Expression 모드)
  ...
  2. 검색 요약에 나온 뉴스 중 가장 중요해 보이는 뉴스 2개를 반드시 선정하여, 'web_scraper'로 작성 날짜 및 상세 내용을 검증하세요.
  
  [중요한 규칙]
  1. 절대 검색 요약 결과만 보고 리포트를 작성하지 마세요. 반드시 'web_scraper'를 통해 검증하세요.
  2. 검색한 적이 있는 키워드는 절대 재사용하지 마세요.
  3. 리포트의 모든 내용은 1년 이내의 최신 자료이며, 반드시 진실이어야 합니다.
