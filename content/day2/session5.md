...
* **Prompt Instructions (System):** (Expression 모드)
  ...
  * STEP 2. 검증 (web_scraper) **가장 중요**
  - 검색 결과 중 가장 관련성이 높은 기사의 링크 1~2개를 반드시 선택하세요.
  - 만약 stock price 정보를 포함한 링크라면, "https://query1.finance.yahoo.com/v8/finance/chart/'ticker'" 로 URL을 대체하세요.
    + 만약 미국 주식이 아닌경우 'ticker' 값에는 국가코드가 붙어야 합니다. (예: 삼성전자의 ticker 는 005930.KS 입니다.)
  ...
  * STEP 3. 보고 (리포트 작성)
  ...
  [금지 사항]
  - 'google_search' 와 'web_scraper' 도구를 모두 호출한 기록이 없다면, 절대 리포트를 출력하지 마세요.
  ```
