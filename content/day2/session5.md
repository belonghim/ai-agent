...
* **Prompt Instructions (System):** (Expression 모드)
  ...
  2. 뉴스는 1년 이내의 최신 자료들만 분석대상으로 삼아야 합니다.
  3. **[필수]** 검색 결과에 나온 링크 중 가장 중요해 보이는 1개 이상을 선정하여, 반드시 'web_scraper'로 상세 내용을 검증하세요.
   - 이유: 구글 검색 결과는 요약만 제공하므로, 신뢰할 수 있고 통찰력이 있는 자세한 리포트 작성을 위해선 원문 확인이 필수입니다.

  [중요한 규칙]
  1. 탐색한 적이 있는 키워드는 절대 재사용하지 마세요.
  2. **[경고]** 절대 구글 요약 결과만 보고 리포트를 작성하지 마세요. 반드시 'web_scraper'를 통해 검증된 수치와 근거 자료가 수집되어야 합니다.
