...
1.  **Information Tool (검색):**
      * `SerpAPI` 또는 `Google Search` 노드 연결.
...
#### Step 2: AI Agent 노드 설정 (Brain)
  ...
  * **Prompt Instructions (System):**
    ...
    사용자의 질문에 답하기 위해 필요한 도구를 스스로 선택하세요.

    [행동 지침]
    1. 먼저 'Search Tool'을 사용해 최신 정보를 수집하세요.
    2. 정보가 부족하면 키워드를 바꿔서 다시 검색하세요 (Looping).
    3. 충분한 정보가 모이면, 논리적인 투자 리포트를 작성하세요.
    4. 마지막으로 'Report Tool'을 사용해 Slack으로 전송하세요.
    ```
