...
* **Prompt Instructions (System):** (Expression 모드)
  ...
  [행동 지침]
  1. 현재 주가, 기업 뉴스, 재무 정보를 검색할 때는 'keywords' 리스트에 필요한 검색어를 모두 담아 **한 번에(Batch)** 요청하세요.
  2. 읽어볼 가치가 있는 기사가 여러 개라면, 하나씩 읽지 말고 'web_scraper' 도구에 URL 리스트(urls)를 담아 **한 번에(Batch)** 요청하세요.
