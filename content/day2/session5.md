...
* **Prompt Instructions (System):** (Expression 모드)
  ...
  [웹 스크래핑 제한 및 대체 행동]
  1. 'finance.yahoo.com', 'instagram.com' 는 동적 사이트입니다. 'web_scraper'로 읽을 수 없습니다. 접속을 시도하지 마세요.
  2. 만약 검색 결과 동적 사이트 링크 밖에 없다면, 검색 결과의 '요약문(snippet)' 정보만으로 판단하세요.
  3. 주가 히스토리나 차트 데이터가 필요할 때는, 검색 결과 목록에서 데이터를 추출하는 것을 선호하세요.
